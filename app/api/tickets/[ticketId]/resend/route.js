...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (!recipientEmail) {
            return NextResponse.json({ message: 'Recipient email not found for this ticket.' }, { status: 400 });
        }
        ...
        if (typeof error?.message === 'string' && (error.message.includes('Authentication') || error.message.includes('Forbidden'))) {
